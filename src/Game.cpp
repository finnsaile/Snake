...
m_highscore(initHighscore(*m_settings_values.getDifficulty()), 40, 35, true)
{
    ...
    m_snake = make_shared<Snake>(START_POS_X, START_POS_Y, *m_settings_values.getLength(), *m_settings_values.getFoodCount());
}
...
WindowInstance Game::gameTick(RenderWindow& renderWindow)
{
    //chang bool to determine if menu should be opened
    m_new_instance = EGame;
    //eventloop
            ...
            switch(event.type)
            {
                ...
                case Event::KeyPressed:
                            switch(event.key.code)
                        {
                            ...
                            case Keyboard::Escape:  m_new_instance = EBreakMenu; break;
                            default: break;
                        }
                default: break;
            }
...
unsigned int Game::initHighscore(Difficulty difficulty)
{
    int arr[5], fst, lst;
    string tempString;
...
float Game::convertDifficulty(Difficulty difficulty)
{
    switch(difficulty)
    {
